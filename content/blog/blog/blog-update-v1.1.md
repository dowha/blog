...
블로그 페이지를 blog.dowha.kim 서브도메인으로 분리해서 연결하고 메인 페이지를 따로 구축했다. 그리고 Netlify에서 제공하는 기본 주소들은 gatsby-plugin-netlify 플러그인으로 _redirects 파일은 만들어 dowha.kim 도메인으로만 연결되도록 설정했다.
...
- [x] 서브도메인 분리
- [x] *.netlify.com 주소 *.dowha.kim으로 redirect 설정
